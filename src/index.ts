...
/**
 * Kansas Statutes Annotated — state statutes by citation.
 *
 * The four-digit section number hides an ARTICLE, as in Pennsylvania: K.S.A.
 * 21-5402 is chapter 21, ARTICLE 54, section 02, and the path wants each part
 * zero-padded to three:
 *
 *   21-5402  ->  /statutes/chapters/ch21/021_054_0002.html
 *                                   ^^^^ ^^^ ^^^ ^^^^
 *
 * Nobody types that, which is the point of a resolver.
 *
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Kansas Statutes Annotated');
}

const BASE = 'https://www.ksrevisor.org/statutes/chapters';
const UA = 'pipeworx-mcp-kansas-code/1.0 (+https://pipeworx.io)';
...
const tools: McpToolExport['tools'] = [{
  ...
  inputSchema: {
    type: 'object',
    properties: {
      section: { type: 'string', description: 'Section as cited, e.g. "21-5402" (murder 1) or "8-1567" (DUI). The digits after the dash encode article and section.' },
    },
    required: ['section'],
  },
}];
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  if (name !== 'ks_statute') throw new Error(`Unknown tool: ${name}`);
  const raw = String(args.section ?? '').trim().replace(/\s+/g, '').replace(/^K\.?S\.?A\.?/i, '');
  const m = /^(\d+)-(\d{2})(\d{1,2})$/.exec(raw);
  if (!m) {
    return { found: false, reason: 'unparsed_section',
      message: `"${args.section}" is not a Kansas citation this can resolve.`,
      hint: 'Kansas sections are chapter-articleNNsection, e.g. "21-5402" (murder 1) — article 54, section 02.' };
  }
  const [, chapter, article, section] = m;
  const url = `${BASE}/ch${chapter}/${chapter.padStart(3, '0')}_${article.padStart(3, '0')}_${section.padStart(4, '0')}.html`;
  const res = await pwFetch(url, { headers: { 'User-Agent': UA } });
  if (res.status === 404) {
    return { found: false, reason: 'section_not_found', section: raw, chapter, article,
      message: `No section ${raw} in the Kansas Statutes (HTTP 404).`,
      hint: 'Section numbers are not contiguous — repealed sections are normal.' };
  }
  if (!res.ok) throw new Error(`Kansas Revisor returned HTTP ${res.status} for ${raw}`);
  const full = statuteText(await readStatuteResponse(res));
  // No content container on this page, so anchor on the heading shape the
  // statute itself uses -- "21-5402. Murder in the first degree." -- which the
  // navigation copies of the number do not match.
  const esc = raw.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const at = full.search(new RegExp(`${esc}\\.\\s+[A-Z]`));
  const text = at === -1 ? full : full.slice(at).trim();
  return {
    citation: `K.S.A. § ${raw}`, section: raw, chapter, article,
    text, text_chars: text.length, url,
    source: 'Kansas Office of Revisor of Statutes (ksrevisor.org), keyless',
    attribution: 'Kansas statutes are public record.',
  };
}
